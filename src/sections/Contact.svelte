...
<section id="Contact" bind:this={contactRef} class="section relative py-24 px-6 md:px-12">
	<!-- Background Glow -->
	<div class="absolute inset-0 bg-gradient-to-br from-[var(--primary-color)] to-[var(--secondary-color)] opacity-15 blur-2xl -z-10"></div>

	<!-- Contact Header -->
	<div class="max-w-5xl mx-auto text-left">
		<h2 in:fly={{ x: -100, duration: 500 }}
		...
		<p in:fly={{ x: -100, duration: 600 }}
			class="text-lg text-white/90 mt-4 max-w-2xl leading-relaxed">
			Have a project in mind or just want to say hi? Fill out the form below, and let’s create something amazing!
		</p>
	</div>

	<!-- Contact Form -->
	<div bind:this={formRef} in:fly={{ y: 100, duration: 600, delay: 300 }}
		...
		<input type="text" bind:value={name} placeholder="Your Name"
			class="w-full px-4 py-3 border-2 border-white/50 bg-black/30 text-white/90 rounded-lg focus:outline-none 
		...
		<textarea bind:value={message} placeholder="Your Message"
			class="w-full px-4 py-3 border-2 border-white/50 bg-black/30 text-white/90 rounded-lg focus:outline-none 
			focus:border-[var(--accent-color)] transition duration-300 mb-4 h-32 placeholder-white/60"></textarea>
